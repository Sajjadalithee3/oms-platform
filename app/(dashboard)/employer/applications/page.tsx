...
interface Application {
  ...
  jobSeeker?: { user: { name: string; email: string } } | null
  learner?: { user: { name: string; email: string } } | null
  _count: { messages: number; interviews: number }
  [key: string]: unknown
}
...
export default function EmployerApplicationsPage() {
  ...
  const columns: Column<Application>[] = [
    { key: "candidate", label: "Candidate", render: (r) => r.jobSeeker?.user.name || r.learner?.user.name || "—" },
  ...
  return (
    <>
      <TopBar title="Applications" notificationCount={0} />
      <div className="p-6">
        <DataTable columns={columns} data={apps} searchPlaceholder="Search applications..." exportFilename="applications" onRowClick={(r) => router.push(`/employer/applications/${r.id}`)} />
      </div>
    </>
  )
}
