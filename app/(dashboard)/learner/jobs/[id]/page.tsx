...
interface Job {
  ...
  sourceType: string; sourceUrl: string | null; status: string
  employer?: { companyName: string } | null
}

export default function LearnerJobDetailPage() {
        ...
        <div className="bg-white rounded-lg border p-6">
          ...
          {skills.length > 0 && (
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-1">
                {skills.map((s) => <Badge key={s} variant={matchedSkills.includes(s) ? "default" : "outline"}>{s}</Badge>)}
              </div>
            </div>
          )}

          {(job.salaryMin || job.salaryMax) && (
            <p className="mt-3 text-sm text-gray-600">Salary: £{job.salaryMin?.toLocaleString() || "?"} - £{job.salaryMax?.toLocaleString() || "?"}</p>
          )}
          ...
          <div className="mt-6">
            {job.status === "CLOSED" ? (
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center text-green-700">You have already applied to this job</div>
            ) : job.sourceType !== "INTERNAL" ? (
              <a href={job.sourceUrl || "#"} target="_blank" rel="noopener noreferrer">
                <Button className="w-full"><ExternalLink className="h-4 w-4 mr-2" />Apply on {job.sourceType}</Button>
              </a>
            ) : (
