...
export default function ProviderAdsPage() {
  ...
  function refresh() {
    fetch("/api/provider/ads").then((r) => r.json()).then((d) => { setAds(d.ads); setUsed(d.used); setCap(d.cap) })
  }
  ...
  async function toggleActive(ad: Ad) {
    await fetch(`/api/provider/ads/${ad.id}`, {
    ...
    <>
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-600">{used} / {cap} ads used this month</p>
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button disabled={atCap}><Plus className="h-4 w-4 mr-2" />New Ad</Button>
            </DialogTrigger>
            <DialogContent>
